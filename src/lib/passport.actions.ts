...
export async function scanPassport(input: unknown): Promise<PassportScanResult> {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        },
        {
          role: "user",
          content: [
            { type: "text", text: "Extract the passport holder details from this passport photo." },
  ...
  if (res.status === 402) throw new Error("Scanning credits exhausted. Enter the details manually.");
