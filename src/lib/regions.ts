import { WORLD_REGIONS } from "./regions.generated";
...
const CURATED_REGIONS: Partial<Record<CountryCode, string[]>> = {
  BD: [
  ...
  SA: [
    ...
    "Jazan", "Najran", "Al Bahah", "Al Jouf", "Qassim",
  ],
  MY: [
...
export const REGIONS: Partial<Record<CountryCode, string[]>> = (() => {
  const merged: Partial<Record<CountryCode, string[]>> = {};
  for (const { code } of COUNTRIES) {
    const list = CURATED_REGIONS[code] ?? WORLD_REGIONS[code];
    if (list && list.length) merged[code] = list;
  }
  return merged;
})();
