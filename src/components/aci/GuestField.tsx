...
export function GuestField({ guests, onChange }: { guests: Guests; onChange: (g: Guests) => void }) {
  ...
  const set = (key: keyof Guests, delta: number) => {
    ...
    const next: Guests = { ...guests, [key]: Math.max(row.min, Math.min(row.max, guests[key] + delta)) };
    ...
    <div ref={boxRef} className="relative min-w-0">
      ...
      {open ? (
        <div className="absolute right-0 top-full z-40 mt-1.5 w-[260px] rounded-xl border border-border/70 bg-popover p-2 shadow-float">
          {rows.map((r) => (
            <div key={r.key} className="flex items-center justify-between rounded-lg px-2 py-2 hover:bg-secondary/50">
              <span>
              ...
              <span className="flex items-center gap-2">
                ...
                <span className="w-5 text-center font-display text-[13px] font-semibold">{guests[r.key]}</span>
                <button
                  type="button"
