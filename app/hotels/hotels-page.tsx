...
export function HotelSearchPage() {
  ...
  const params = useMemo(
    () => validateHotelSearch(searchParamsToRecord(new URLSearchParams(searchParams.toString()))),
    [searchParams],
  );
      ...
      <div className="border-b border-border/60 bg-card">
        <div className="mx-auto max-w-[1240px] px-3 py-3 sm:px-5">
          <h1 className="sr-only">
            Hotels in {dest ? dest.label : "your destination"}
          </h1>
          <button
            type="button"
            onClick={() => setEdit((v) => !v)}
          ...
          <>
            <div className="flex items-start gap-3 border-b border-border/60 p-4">
              <div className="min-w-0 flex-1">
                ...
                <h2 className="mt-0.5 truncate font-display text-[17px] font-bold">{picked.hotel.name}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              ...
              <div className="mt-4 space-y-2 text-[12.5px]">
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground">
                    {bdt(picked.room.perNight)} × {nights} night{nights > 1 ? "s" : ""} × {rooms} room
                    {rooms > 1 ? "s" : ""}
                  </span>
              ...
              <ul className="mt-4 space-y-1.5 text-[12px] text-muted-foreground">
                <li className="flex items-center gap-1.5">
                  ...
                  {picked.room.refundable ? "Free cancellation until 24h before check-in" : "Non-refundable rate"}
                </li>
                <li className="flex items-center gap-1.5">
