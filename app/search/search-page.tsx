...
import { searchParamsToRecord, withSearch } from "@/lib/navigation";
import {
  decodeLegs,
  encodeLegs,
  legLabelFor,
...
export function SearchPage() {
  ...
  const params = useMemo(
    () => validateFlightSearch(searchParamsToRecord(new URLSearchParams(searchParams.toString()))),
    [searchParams],
  );
  const router = useRouter();
  const [edit, setEdit] = useState(false);
  const [loading, setLoading] = useState(true);
  const [holdSeconds, setHoldSeconds] = useState(20 * 60);
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setHoldSeconds((value) => (value > 0 ? value - 1 : 20 * 60)), 1000);
  ...
  const goToDate = (nextDate: string) => {
    ...
    const delta = Math.round(
      (new Date(`${nextDate}T00:00:00`).getTime() - new Date(`${legs[0]!.date}T00:00:00`).getTime()) /
        86400000,
    );
    if (!delta) return;
    const shifted = legs.map((l) => ({ from: l.from, to: l.to, date: shiftDays(l.date, delta) }));
    router.push(withSearch("/search", { ...params, legs: encodeLegs(shifted) }));
  };

  const dateStrip = legs.length
    ? [-2, -1, 0, 1, 2].map((o) => shiftDays(legs[0]!.date, o))
    : [];
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border/60 bg-card">
        <div className="mx-auto max-w-[1240px] px-3 py-3 sm:px-5">
          ...
          <button
            ...
            onClick={() => setEdit((v) => !v)}
            aria-expanded={edit}
            className="group flex w-full items-stretch gap-0 overflow-hidden rounded-xl border border-border bg-background text-left shadow-soft transition hover:border-primary/40 hover:shadow-float"
          >
            <span className="flex min-w-0 flex-1 items-center gap-3 px-3 py-2.5 sm:px-4">
              ...
              <span className="min-w-0 flex-1">
                <span className="flex min-w-0 items-center gap-2 font-display text-[14px] font-semibold leading-tight sm:text-[15.5px]">
                  <span className="truncate">
                  {route}
                  </span>
                  {legs.length ? (
                    <span className="shrink-0 text-[10.5px] font-semibold text-teal">9 fares available</span>
                  ) : null}
                </span>
            ...
            <span className="mr-2 flex shrink-0 items-center gap-1.5 self-center rounded-lg bg-forest px-3.5 py-2 text-[12px] font-semibold text-primary-foreground shadow-soft transition group-hover:scale-[1.02] sm:mr-3">
      ...
      <main className="mx-auto max-w-[1240px] px-2.5 py-3 sm:px-5 sm:py-5">
        {edit || legs.length === 0 ? (
        ...
        {legs.length === 0 ? (
          ...
          <div className={cn(
            "mb-2.5 grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-1 rounded-lg border border-border/60 bg-card p-1 shadow-soft sm:mb-4 sm:gap-1.5 sm:rounded-xl sm:p-1.5",
            "md:grid-cols-[auto_auto_minmax(0,1fr)_auto_auto]",
          )}>
            <span className="hidden items-center gap-1.5 rounded-lg bg-secondary px-3 py-2 font-display text-[12px] font-semibold text-primary md:flex">
              ...
              {dateStrip.map((d) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => goToDate(d)}
                    className={cn(
                       "flex-1 whitespace-nowrap rounded-lg px-2 py-1 text-center transition sm:rounded-xl sm:px-3 sm:py-1.5",
                      active
                  ...
                  >
                    ...
                    <span className="block font-display text-[12.5px] font-semibold">{prettyDate(d)}</span>
                  </button>
                );
              })}
            </div>
            ...
            <button
              type="button"
              onClick={() => document.getElementById(legs.length > 1 ? "itineraries" : "leg-1")?.scrollIntoView({ behavior: "smooth" })}
              className="hidden shrink-0 items-center gap-1.5 rounded-lg bg-secondary px-3 py-2 text-[11.5px] font-semibold text-primary md:flex"
            >
              <ShieldCheck className="size-3.5" /> Fare filters
            </button>
          </div>
        )}
