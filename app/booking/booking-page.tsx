...
import { scanPassport } from "@/lib/passport.actions";
import { withSearch } from "@/lib/navigation";
import { cn } from "@/lib/utils";

type Draft = { legs: ItineraryLeg[]; pax: Pax; cabin: string; channel?: "b2c" | "agent" };
...
const extras = [
  { id: "bag", icon: Luggage, name: "Extra checked bag", desc: "+10 kg allowance per passenger", price: 3200 },
  { id: "seat", icon: Ticket, name: "Preferred seat", desc: "Window or aisle, extra legroom rows", price: 1800 },
  { id: "meal", icon: Utensils, name: "Premium meal", desc: "Chef-curated hot meal & dessert", price: 1200 },
  { id: "flex", icon: ShieldCheck, name: "Flex protection", desc: "Free one-time date change", price: 2400 },
];
...
export function BookingPage() {
  ...
  const isAgent = draft?.channel === "agent";
  const listTotal = legs.reduce(
    (t, l) => t + (isAgent ? (l.fare.customerPrice ?? l.fare.price) : flightFacts(l.fare).listPrice),
    0,
  );
  const extrasTotal = useMemo(
    () => picked.reduce((t, id) => t + (extras.find((e) => e.id === id)?.price ?? 0) * travellerCount, 0),
  ...
  const canContinue =
    step !== 0 ||
    (contact.email.includes("@") && contact.phone.length > 5 && travellers.every((t) => t.first && t.last));
              ...
              <li key={s} className="flex flex-1 items-center gap-2">
                <span
                  className={cn(
                    ...
                    i <= step ? "bg-forest text-primary-foreground" : "bg-secondary text-muted-foreground",
                  )}
                >
          ...
          <div className="space-y-4">
            {done ? (
              <section className="surface-card p-6 text-center">
                ...
                <h2 className="mt-3 font-display text-[19px] font-bold">You&apos;re all set, safe travels!</h2>
                <p className="mt-1 text-[12.5px] text-muted-foreground">
                ...
                <div className="mt-5 flex flex-wrap justify-center gap-2">
                  ...
                  <Link href="/" className="rounded-xl border border-border px-4 py-2.5 text-[13px] font-semibold">
                    Back to home
                  </Link>
                </div>
              </section>
              ...
              <>
                ...
                {travellers.map((t, i) => (
                  <TravellerCard
                    ...
                    onChange={(patch) =>
                      setTravellers((list) => list.map((x, j) => (j === i ? { ...x, ...patch } : x)))
                    }
                  />
                ))}
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="min-w-0 flex-1">
                          <span className="block font-display text-[13.5px] font-semibold">{e.name}</span>
                          <span className="block text-[11.5px] text-muted-foreground">{e.desc}</span>
                ...
                <section className="surface-card p-4 sm:p-5">
                  ...
                  <div className="mt-3 space-y-3">
                    {legs.map((l, i) => {
                      ...
                      return (
                        <div key={i} className="rounded-2xl border border-border/70 bg-secondary/30 p-3.5">
                          ...
                          <div className="mt-3 flex items-center gap-3">
                            <div>
                              <p className="font-display text-[18px] font-bold leading-none">{l.fare.depart}</p>
                            ...
                            <div className="flex-1">
                              ...
                              <p className="mt-1.5 text-center text-[10.5px] font-semibold text-muted-foreground">
                                {l.fare.duration} · {l.fare.stops === 0 ? "Non-stop" : `${l.fare.stops} stop`}
                              </p>
                            </div>
                            <div className="text-right">
                              <p className="font-display text-[18px] font-bold leading-none">{l.fare.arrive}</p>
                              <p className="mt-1 text-[11px] text-muted-foreground">
                    ...
                    {["Card / 3-D Secure", "bKash · Nagad", "0% EMI · 12 banks"].map((m, i) => (
                      <div
                        ...
                        className={cn(
                          ...
                          i === 0 ? "border-primary bg-primary/5" : "border-border bg-card text-muted-foreground",
                        )}
                      >
          ...
          <aside className="lg:sticky lg:top-[74px] lg:self-start">
            <div className="surface-card overflow-hidden">
              <div className="bg-forest px-4 py-3 text-primary-foreground">
                <p className="text-[10px] font-semibold uppercase tracking-[0.2em] opacity-80">Price summary</p>
                <p className="mt-1 font-display text-[24px] font-bold leading-none">{money(grandTotal)}</p>
              ...
              <div className="space-y-2 px-4 py-4">
                {legs.map((l, i) => (
                  <div key={i}>
                    ...
                    {fareBreakdown(l.fare, pax).map((r) => (
                      <div key={r.label} className="flex items-center justify-between gap-3 py-0.5 text-[12px]">
                ...
                <div className="flex items-center justify-between gap-3 border-t border-border/70 pt-2 text-[12px]">
                  <span className="text-muted-foreground">{isAgent ? "Agent commission" : "You save"}</span>
                  <span className="font-semibold text-primary">-{money(Math.max(listTotal - flightTotal, 0))}</span>
                </div>
            ...
            <div className="mt-3 grid gap-2 rounded-2xl border border-border/60 bg-secondary/40 p-3.5">
              {[
                ...
                <p key={x.t} className="flex items-center gap-2 text-[11.5px] text-muted-foreground">
...
function TravellerCard({
  ...
  const handleFile = async (file: File | undefined) => {
    ...
    try {
      ...
      onChange({ scan: image });
      const result = await scanPassport({ image });
      onChange({
      ...
      <div className="p-4 sm:p-5">
        <div className="flex flex-col gap-3 rounded-2xl border border-primary/20 bg-primary/5 p-3.5 sm:flex-row sm:items-center">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-display text-[13.5px] font-semibold">Scan passport &amp; auto-fill</p>
            <p className="mt-0.5 text-[11.5px] text-muted-foreground">
              {busy
