...
import { ALL_COUNTRIES, CountryDropdown } from "@/components/ui/country-dropdown";
...
const partnerInfoSchema = z.object({
  ...
  postalCode: z.string().trim().min(2, "Postal code is required").max(12, "Postal code is too long"),
  ...
  if (country && /^\d+$/.test(data.phone)) {
    ...
    if (!phoneNumber || phoneNumber.country !== country.alpha3 || !phoneNumber.isValid()) {
      ctx.addIssue({
        code: "custom",
        path: ["phone"],
...
export function RegistrationForm() {
  ...
  const selectedCountryCode = ALL_COUNTRIES.find((country) => country.name === selectedCountryName)?.alpha3;
  ...
  const onStep1Submit = async (data: PartnerInfo) => {
    ...
    try {
      ...
      if (!response.ok) {
        partnerForm.setError("postalCode", {
  ...
  return (
    <div className="registration-form flex flex-col gap-10 w-full">
      ...
      <div className="registration-panel bg-white/40 border border-white/60 rounded-3xl p-6 md:p-8 shadow-sm">
        ...
        {step === 1 && (
          <form noValidate onSubmit={partnerForm.handleSubmit(onStep1Submit)} className="registration-profile-form flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              >
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-xs">
                    {profileErrorMessages.length} {profileErrorMessages.length === 1 ? "field needs" : "fields need"} your attention.
                  </p>
                </div>
              </div>
            )}
            <div className="registration-fields grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
                ...
                <Input id="ownerName" {...partnerForm.register("ownerName")} aria-invalid={!!partnerForm.formState.errors.ownerName} aria-describedby={partnerForm.formState.errors.ownerName ? "ownerName-error" : undefined} />
                {partnerForm.formState.errors.ownerName && <span id="ownerName-error" role="alert" className="text-xs text-red-600 ml-1 font-medium">{partnerForm.formState.errors.ownerName.message}</span>}
              </div>
              <div className="flex flex-col gap-2">
                ...
                <Input id="companyName" {...partnerForm.register("companyName")} aria-invalid={!!partnerForm.formState.errors.companyName} aria-describedby={partnerForm.formState.errors.companyName ? "companyName-error" : undefined} />
                {partnerForm.formState.errors.companyName && <span id="companyName-error" role="alert" className="text-xs text-red-600 ml-1 font-medium">{partnerForm.formState.errors.companyName.message}</span>}
              </div>
              <div className="flex flex-col gap-2">
                <Label htmlFor="caabNumber" className="text-slate-700 font-semibold ml-1">CAAB Number <span className="text-xs font-normal text-slate-400">(Optional)</span></Label>
                <Input id="caabNumber" placeholder="Enter CAAB number" {...partnerForm.register("caabNumber")} aria-invalid={!!partnerForm.formState.errors.caabNumber} aria-describedby={partnerForm.formState.errors.caabNumber ? "caabNumber-error" : undefined} />
                {partnerForm.formState.errors.caabNumber && <span id="caabNumber-error" role="alert" className="text-xs text-red-600 ml-1 font-medium">{partnerForm.formState.errors.caabNumber.message}</span>}
              </div>
              <div className="flex flex-col gap-2">
                <Label htmlFor="email" className="text-slate-700 font-semibold ml-1">Partner Email <span className="text-red-500">*</span></Label>
                <Input id="email" type="email" {...partnerForm.register("email")} aria-invalid={!!partnerForm.formState.errors.email} aria-describedby={partnerForm.formState.errors.email ? "email-error" : undefined} />
                {partnerForm.formState.errors.email && <span id="email-error" role="alert" className="text-xs text-red-600 ml-1 font-medium">{partnerForm.formState.errors.email.message}</span>}
              </div>
              <div className="flex flex-col gap-2">
                <Label htmlFor="password" className="text-slate-700 font-semibold ml-1">Password <span className="text-red-500">*</span></Label>
                <Input id="password" type="password" {...partnerForm.register("password")} aria-invalid={!!partnerForm.formState.errors.password} aria-describedby={partnerForm.formState.errors.password ? "password-error" : undefined} />
                {partnerForm.formState.errors.password && <span id="password-error" role="alert" className="text-xs text-red-600 ml-1 font-medium">{partnerForm.formState.errors.password.message}</span>}
              </div>
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <Input id="phone" type="tel" inputMode="numeric" autoComplete="tel-national" className="w-2/3" placeholder="National number" {...partnerForm.register("phone")} aria-invalid={!!partnerForm.formState.errors.phone} aria-describedby={(partnerForm.formState.errors.phone || partnerForm.formState.errors.dialCode) ? "phone-error" : undefined} />
                </div>
                {(partnerForm.formState.errors.phone || partnerForm.formState.errors.dialCode) && <span id="phone-error" role="alert" className="text-xs text-red-600 ml-1 font-medium">{partnerForm.formState.errors.phone?.message || partnerForm.formState.errors.dialCode?.message}</span>}
              </div>
              <div className="flex flex-col gap-2">
                  ...
                  render={({ field }) => (
                    <CountryDropdown
                      ...
                      onChange={(country) => {
                        ...
                        partnerForm.setValue("city", "", { shouldDirty: true, shouldValidate: false });
                        partnerForm.setValue("postalCode", "", { shouldDirty: true, shouldValidate: false });
                ...
                />
                {partnerForm.formState.errors.country && <span id="country-error" role="alert" className="text-xs text-red-600 ml-1 font-medium">{partnerForm.formState.errors.country.message}</span>}
              </div>
              <div className="flex flex-col gap-2">
                ...
                />
                {partnerForm.formState.errors.city && <span id="city-error" role="alert" className="text-xs text-red-600 ml-1 font-medium">{partnerForm.formState.errors.city.message}</span>}
              </div>
              <div className="flex flex-col gap-2 md:col-span-2">
                <Label htmlFor="address" className="text-slate-700 font-semibold ml-1">Office Address <span className="text-red-500">*</span></Label>
                <Input id="address" autoComplete="street-address" {...partnerForm.register("address")} aria-invalid={!!partnerForm.formState.errors.address} aria-describedby={partnerForm.formState.errors.address ? "address-error" : undefined} />
                {partnerForm.formState.errors.address && <span id="address-error" role="alert" className="text-xs text-red-600 ml-1 font-medium">{partnerForm.formState.errors.address.message}</span>}
              </div>
              <div className="flex flex-col gap-2">
                <Label htmlFor="postalCode" className="text-slate-700 font-semibold ml-1">Postal Code <span className="text-red-500">*</span></Label>
                <Input id="postalCode" autoComplete="postal-code" {...partnerForm.register("postalCode")} aria-invalid={!!partnerForm.formState.errors.postalCode} aria-describedby={partnerForm.formState.errors.postalCode ? "postalCode-error" : undefined} />
                {partnerForm.formState.errors.postalCode && <span id="postalCode-error" role="alert" className="text-xs text-red-600 ml-1 font-medium">{partnerForm.formState.errors.postalCode.message}</span>}
              </div>
              <div className="flex flex-col gap-2">
                <Label htmlFor="currency" className="text-slate-700 font-semibold ml-1">Operating Currency <span className="text-red-500">*</span></Label>
                <Select id="currency" {...partnerForm.register("currency")} aria-invalid={!!partnerForm.formState.errors.currency} aria-describedby={partnerForm.formState.errors.currency ? "currency-error" : undefined}>
                ...
                {partnerForm.formState.errors.currency && <span id="currency-error" role="alert" className="text-xs text-red-600 ml-1 font-medium">{partnerForm.formState.errors.currency.message}</span>}
              </div>
            </div>
            <div className="registration-actions flex justify-end mt-4">
              <Button type="submit" disabled={isAddressVerifying} className="bg-gradient-to-r from-aci-green-500 to-aci-green-700 hover:from-aci-green-700 hover:to-aci-green-900 text-white w-full md:w-auto h-12 px-8 rounded-xl shadow-lg shadow-aci-green-500/20 font-semibold tracking-wide transition-all duration-300">
        ...
        {step === 2 && (
          <div className="flex flex-col gap-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className={`group relative min-w-0 overflow-hidden rounded-2xl border-2 border-dashed p-4 transition-all duration-300 ${docErrors.nid ? 'border-red-400 bg-red-50/50' : documents.nid ? 'border-aci-green-500 bg-aci-green-50/50' : 'border-slate-300 bg-white/50 hover:border-aci-green-400 hover:bg-white/80 hover:shadow-md'}`}>
              ...
              <div className="pointer-events-none flex h-full flex-col">
                <div className="flex items-center gap-3">
                  <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-colors ${documents.nid ? 'bg-aci-green-100 text-aci-green-600' : 'bg-white text-slate-400 shadow-sm group-hover:text-aci-green-500'}`}>
                    {documents.nid ? <Check size={20} strokeWidth={2.5} /> : <Upload size={20} />}
                  </div>
                  <h3 className="font-bold text-slate-800">National ID <span className="text-red-500">*</span></h3>
                </div>
                <p className="mt-3 text-xs leading-5 text-slate-500">Owner&apos;s clear scanned ID or photo.</p>
                {documents.nid ? (
                  <div className="mt-3 flex min-w-0 items-center gap-2 rounded-lg border border-aci-green-200 bg-white px-3 py-2">
                    ...
                    <span className="truncate text-xs font-semibold text-slate-700">{documents.nid.name}</span>
                  </div>
                ) : (
                  <span className="mt-3 text-xs font-bold text-aci-green-600">Choose file</span>
                )}
                {docErrors.nid && <p role="alert" className="mt-2 text-xs font-bold text-red-600">{docErrors.nid}</p>}
              </div>
            </div>

            <div className={`group relative min-w-0 overflow-hidden rounded-2xl border-2 border-dashed p-4 transition-all duration-300 ${docErrors.tradeLicense ? 'border-red-400 bg-red-50/50' : documents.tradeLicense ? 'border-aci-green-500 bg-aci-green-50/50' : 'border-slate-300 bg-white/50 hover:border-aci-green-400 hover:bg-white/80 hover:shadow-md'}`}>
              ...
              <div className="pointer-events-none flex h-full flex-col">
                <div className="flex items-center gap-3">
                  <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-colors ${documents.tradeLicense ? 'bg-aci-green-100 text-aci-green-600' : 'bg-white text-slate-400 shadow-sm group-hover:text-aci-green-500'}`}>
                    {documents.tradeLicense ? <Check size={20} strokeWidth={2.5} /> : <Upload size={20} />}
                  </div>
                  <h3 className="font-bold text-slate-800">Trade License <span className="text-red-500">*</span></h3>
                </div>
                <p className="mt-3 text-xs leading-5 text-slate-500">Your company&apos;s valid current license.</p>
                {documents.tradeLicense ? (
                  <div className="mt-3 flex min-w-0 items-center gap-2 rounded-lg border border-aci-green-200 bg-white px-3 py-2">
                    <File size={14} className="shrink-0 text-aci-green-500" />
                    <span className="truncate text-xs font-semibold text-slate-700">{documents.tradeLicense.name}</span>
                  </div>
                ) : (
                  <span className="mt-3 text-xs font-bold text-aci-green-600">Choose file</span>
                )}
                {docErrors.tradeLicense && <p role="alert" className="mt-2 text-xs font-bold text-red-600">{docErrors.tradeLicense}</p>}
              </div>
            </div>

            <div className={`group relative min-w-0 overflow-hidden rounded-2xl border-2 border-dashed p-4 transition-all duration-300 ${docErrors.caabCertificate ? 'border-red-400 bg-red-50/50' : documents.caabCertificate ? 'border-aci-green-500 bg-aci-green-50/50' : 'border-slate-300 bg-white/50 hover:border-aci-green-400 hover:bg-white/80 hover:shadow-md'}`}>
              ...
              <div className="pointer-events-none flex h-full flex-col">
                <div className="flex items-center gap-3">
                  <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-colors ${documents.caabCertificate ? 'bg-aci-green-100 text-aci-green-600' : 'bg-white text-slate-400 shadow-sm group-hover:text-aci-green-500'}`}>
                    {documents.caabCertificate ? <Check size={20} strokeWidth={2.5} /> : <Upload size={20} />}
                  </div>
                  <div>
                ...
                <p className="mt-3 text-xs leading-5 text-slate-500">Civil Aviation Authority certificate.</p>
                {documents.caabCertificate ? (
                  <div className="mt-3 flex min-w-0 items-center gap-2 rounded-lg border border-aci-green-200 bg-white px-3 py-2">
                    <File size={14} className="shrink-0 text-aci-green-500" />
                    <span className="truncate text-xs font-semibold text-slate-700">{documents.caabCertificate.name}</span>
                  </div>
                ) : (
                  <span className="mt-3 text-xs font-bold text-slate-500 group-hover:text-aci-green-600">Choose file</span>
                )}
                {docErrors.caabCertificate && <p role="alert" className="mt-2 text-xs font-bold text-red-600">{docErrors.caabCertificate}</p>}
              </div>
            </div>
            </div>
            ...
            <div className="registration-review-profile grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="registration-review-card bg-white/70 border border-slate-200 rounded-2xl p-6 shadow-sm">
                ...
                <div className="flex flex-col gap-4 text-sm">
                  ...
                  {partnerForm.getValues("caabNumber") && <div className="flex flex-col"><span className="text-slate-500 text-xs uppercase tracking-wider mb-1">CAAB Number</span> <span className="font-semibold text-slate-800">{partnerForm.getValues("caabNumber")}</span></div>}
                  <div className="flex flex-col"><span className="text-slate-500 text-xs uppercase tracking-wider mb-1">Operating Currency</span> <span className="font-semibold text-slate-800">{partnerForm.getValues("currency")}</span></div>
                </div>
              </div>
              ...
              <div className="registration-review-card bg-white/70 border border-slate-200 rounded-2xl p-6 shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                  <div className="flex min-w-0 flex-col"><span className="text-slate-500 text-xs uppercase tracking-wider mb-1">Email</span> <span className="font-semibold text-slate-800 break-all">{partnerForm.getValues("email")}</span></div>
                  <div className="flex flex-col"><span className="text-slate-500 text-xs uppercase tracking-wider mb-1">Phone</span> <span className="font-semibold text-slate-800">{partnerForm.getValues("dialCode")} {partnerForm.getValues("phone")}</span></div>
                  ...
                  <div className="flex flex-col"><span className="text-slate-500 text-xs uppercase tracking-wider mb-1">Postal Code</span> <span className="font-semibold text-slate-800">{partnerForm.getValues("postalCode")}</span></div>
                  <div className="flex min-w-0 flex-col sm:col-span-2"><span className="text-slate-500 text-xs uppercase tracking-wider mb-1">Office Address</span> <span className="font-semibold text-slate-800 break-words">{partnerForm.getValues("address")}</span></div>
                </div>
              </div>
            </div>
