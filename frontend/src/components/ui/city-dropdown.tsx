...
export function CityDropdown({ countryCode, value, onChange, "aria-invalid": ariaInvalid }: CityDropdownProps) {
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      >
        ...
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={cn("shrink-0 text-slate-400 transition-transform", isOpen && "rotate-180")}>
          <path d="m6 9 6 6 6-6" />
        </svg>
      </button>
      ...
      {isOpen && (
        <div className="country-dropdown-menu absolute z-50 mt-1 w-full overflow-hidden rounded-xl border border-slate-200 bg-white p-2 shadow-lg">
          ...
          <div className="country-dropdown-list max-h-52 overflow-y-auto overscroll-contain" role="listbox">
            ...
            {filteredCities.length === 0 && <p className="px-3 py-3 text-sm text-slate-500">No cities found</p>}
          </div>
        </div>
      )}
    </div>
